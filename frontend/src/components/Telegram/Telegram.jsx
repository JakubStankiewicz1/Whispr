...
const Telegram = ({ 
  ...
  // Format message time for Telegram
  ...
  return (
    <div className={`telegram ${selectedDevice === 'mobile' ? 'telegram--mobile' : 'telegram--desktop'} ${darkMode ? 'telegram--dark' : ''}`}>
      <div className="telegramContainer">
        {/* Header */}
        {showHeader && (
          <div className="telegramHeader">
            <div className="telegramHeaderContainer">
              <div className="telegramHeaderLeft">
                <div className="telegramHeaderAvatar">
                  {getHeaderImage() ? (
                    <img 
                      src={getHeaderImage()} 
                      alt="Avatar" 
                      className="telegramHeaderAvatarImage"
                    />
                  ) : (
                    <span className="telegramHeaderAvatarText">{getHeaderInitial()}</span>
                  )}
                </div>
                <div className="telegramHeaderInfo">
                  <h3 className="telegramHeaderName">{getHeaderName()}</h3>
                  <p className="telegramHeaderStatus">{getHeaderStatus()}</p>
                </div>
              </div>
              <div className="telegramHeaderRight">
                <button className="telegramHeaderButton">
                  <FiSearch className="telegramHeaderIcon" />
                </button>
                <button className="telegramHeaderButton">
                  <FiPhone className="telegramHeaderIcon" />
                </button>
                <button className="telegramHeaderButton">
                  <FiVideo className="telegramHeaderIcon" />
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Messages Area */}
        <div className="telegramMessages">
          <div className="telegramMessagesContainer">
            {messages && messages.length > 0 ? (
              <div className="telegramMessagesList">
                {messages.map((message, index) => {
                  ...
                  return (
                    <React.Fragment key={message.id || index}>
                      {showDateSeparator && (
                        <div className="telegramDateSeparator">
                          <span className="telegramDateText">
                            {formatMessageDate(getMessageDate(message))}
                          </span>
                        </div>
                      )}
                      <div className={`telegramMessage ${isSender ? 'telegramMessage--sender' : 'telegramMessage--receiver'}`}>
                        {!isSender && (
                          <div className="telegramMessageAvatar">
                            {receiverImages && receiverImages[0] ? (
                              <img 
                                src={receiverImages[0]} 
                                alt="Avatar" 
                                className="telegramMessageAvatarImage"
                              />
                            ) : (
                              <div className="telegramMessageAvatarFallback">
                                <span className="telegramMessageAvatarIcon">
                                  {getReceiverInitial()}
                                </span>
                              </div>
                            )}
                        ...
                        <div className={`telegramMessageBubble ${isSender ? 'telegramMessageBubble--sender' : 'telegramMessageBubble--receiver'}`}>
                          {message.images && message.images.length > 0 && (
                            <div className="telegramMessageImages">
                              {message.images.map((image, imgIndex) => (
                                <div 
                                  key={imgIndex} 
                                  className="telegramMessageImageWrapper"
                                  onClick={() => handleImageClick(image)}
                                >
                                  <img 
                                    src={image} 
                                    alt="Message" 
                                    className="telegramMessageImage"
                                  />
                                </div>
                              ))}
                            </div>
                          ...
                          {message.text && (
                            <p className="telegramMessageText">{message.text}</p>
                          )}
                          <span className="telegramMessageTimestamp">
              ...
              <div className="telegramEmptyState">
                <BsTelegram className="telegramEmptyIcon" />
                <p className="telegramEmptyText">Start a conversation</p>
              </div>
            )}
          </div>
        </div>
        ...
        {showFooter && (
          <div className="telegramInput">
            <div className="telegramInputContainer">
              <div className="telegramInputLeft">
                <button className="telegramInputButton" onClick={handleVoiceRecord}>
                  <HiOutlineMicrophone className={`telegramInputIcon ${isRecording ? 'recording' : ''}`} />
                </button>
              </div>
              <div className="telegramInputCenter">
                <input
                  type="text"
                  className="telegramInputField"
              ...
              <div className="telegramInputRight">
                <button className="telegramInputButton">
                  <FiImage className="telegramInputIcon" />
                </button>
                <button className="telegramSendButton" onClick={handleSendMessage}>
                  <IoSend className="telegramSendIcon" />
                </button>
              </div>
            </div>
          </div>
      ...
      {/* Image Modal */}
      {selectedImage && (
        <div className="telegramImageModal" onClick={closeImageModal}>
          <div className="telegramImageModalContent" onClick={(e) => e.stopPropagation()}>
            <img 
              src={selectedImage} 
              alt="Full size" 
              className="telegramImageModalImage"
            />
            <button className="telegramImageModalClose" onClick={closeImageModal}>
              <IoCloseOutline className="telegramImageModalCloseIcon" />
            </button>
          </div>
        </div>
      )}
